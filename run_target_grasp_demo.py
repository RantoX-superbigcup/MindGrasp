...
def call_groundingdino_for_bbox(
    ...
    try:
        ...
        raise RuntimeError(
            "GroundingDINO is not installed. Use --localizer qwen_bbox for now, "
            "or install GroundingDINO and pass --groundingdino-config/--groundingdino-checkpoint."
        ) from exc
    from PIL import Image
...
def call_sam_for_mask(
    ...
    try:
        ...
        raise RuntimeError(
            "segment-anything is not installed. Use --segmenter none for now, "
            "or install SAM and pass --sam-checkpoint."
        ) from exc
    from PIL import Image
...
def resolve_target_region(rgb_path: Path, selected: Dict[str, Any], args: argparse.Namespace, output_dir: Path) -> Dict[str, Any]:
    ...
    if args.mask:
        ...
        if not api_key:
            raise RuntimeError(f"--refine-qwen-bbox needs API key. Set: $env:{args.api_key_env}=\"your_key\"")
...
def main() -> None:
    ...
    if args.bbox:
        ...
        options_result = call_qwen_for_options(rgb_path, args.qwen_base_url, args.qwen_model, api_key, args.max_options)

...
def _furthest_point_sample_pytorch(xyz, npoint):
    B, N, _ = xyz.shape
...
def _gather_operation_pytorch(features, idx):
    idx = idx.long()
    B, C, _ = features.shape
    return torch.gather(features, 2, idx.unsqueeze(1).expand(B, C, -1))


def _grouping_operation_pytorch(features, idx):
    idx = idx.long()
    B, C, _ = features.shape
    _, npoint, nsample = idx.shape
    flat_idx = idx.reshape(B, -1)
    grouped = torch.gather(features, 2, flat_idx.unsqueeze(1).expand(B, C, -1))
...
def _three_interpolate_pytorch(features, idx, weight):
    grouped = _grouping_operation_pytorch(features, idx)
...
def _ball_query_pytorch(radius, nsample, xyz, new_xyz):
    ...
    top_dist, idx = torch.topk(dist, k=k, dim=2, largest=False)
    fallback = idx[:, :, :1].expand(-1, -1, k)
    idx = torch.where(top_dist <= radius, idx, fallback)
    if k < nsample:
        pad = idx[:, :, -1:].expand(-1, -1, nsample - k)
        idx = torch.cat([idx, pad], dim=2)
    return idx.long()


def _cylinder_query_pytorch(radius, hmin, hmax, nsample, xyz, new_xyz, rot):
    B, N, _ = xyz.shape
    ...
    idx_batches = []
    for b in range(B):
        rel = xyz[b].unsqueeze(0) - new_xyz[b].unsqueeze(1)
        local = torch.einsum("pnc,pcd->pnd", rel, rot[b])
        radial2 = local[..., 1] ** 2 + local[..., 2] ** 2
        valid = (local[..., 0] >= hmin) & (local[..., 0] <= hmax) & (radial2 <= radius ** 2)
        metric = torch.where(valid, radial2, torch.full_like(radial2, float("inf")))
        top_metric, idx = torch.topk(metric, k=k, dim=1, largest=False)
        nearest = torch.topk(torch.sum(rel ** 2, dim=-1), k=k, dim=1, largest=False).indices
        idx = torch.where(torch.isfinite(top_metric), idx, nearest)
        if k < nsample:
            pad = idx[:, -1:].expand(-1, nsample - k)
            idx = torch.cat([idx, pad], dim=1)
        idx_batches.append(idx)
    return torch.stack(idx_batches, dim=0).long()
...
class FurthestPointSampling(Function):
    ...
    def forward(ctx, xyz, npoint):
        ...
        """
        if _ext is None:
            return _furthest_point_sample_pytorch(xyz, npoint)
...
class GatherOperation(Function):
    ...
    def forward(ctx, features, idx):
        ...
        if _ext is None:
            return _gather_operation_pytorch(features, idx)
...
class GroupingOperation(Function):
    ...
    def forward(ctx, features, idx):
        ...
        if _ext is None:
            return _grouping_operation_pytorch(features, idx)
...
class CylinderQuery(Function):
    ...
    def forward(ctx, radius, hmin, hmax, nsample, xyz, new_xyz, rot):
        ...
        """
        if _ext is None:
            return _cylinder_query_pytorch(radius, hmin, hmax, nsample, xyz, new_xyz, rot)

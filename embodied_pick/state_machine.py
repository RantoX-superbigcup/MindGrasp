...
class EmbodiedPickPipeline:
    def __init__(self, config: FrameworkConfig) -> None:
        ...
        self.qwen = QwenVisionClient(config.qwen_base_url, config.qwen_model, config.qwen_api_key, config.qwen_dry_run)
        self.graspnet = GraspNetAdapter(
            config.graspnet_root,
            config.graspnet_checkpoint,
            dry_run=config.graspnet_dry_run,
            collision_thresh=config.collision_thresh,
            top_k=config.top_k_grasps,
        )
        self.planner = GraspPlanner(dry_run=config.planner_dry_run)
        self.robot = DryRunRobotController() if config.robot_dry_run else SerialArmController(
            config.robot_port,

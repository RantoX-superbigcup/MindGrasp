...
from embodied_pick.config import load_config
from embodied_pick.state_machine import EmbodiedPickPipeline
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.embodied_pipeline = self.init_embodied_pipeline()
    ...
    def init_ui(self):
        ...
        self.content_label = QLabel("具身抓取框架已加载，等待脑机指令")
        self.content_label.setStyleSheet("color: black; font-size: 16px;")
    ...
    # 初始化具身抓取框架
    def init_embodied_pipeline(self):
        try:
            config_path = Path(__file__).resolve().parents[1] / "configs" / "default_config.json"
            return EmbodiedPickPipeline(load_config(config_path))
        except Exception as exc:
            print(f"embodied pipeline init failed: {exc}")
            return None

    # 将平台输出的脑机指令送入具身抓取状态机
    def handle_embodied_command(self, result):
        if self.embodied_pipeline is None:
            if hasattr(self, "content_label"):
                self.content_label.setText("具身抓取框架初始化失败，请检查 configs/default_config.json")
            return
        context = self.embodied_pipeline.handle_bci_command(result)
        if hasattr(self, "content_label"):
            target = context.target_label or self.embodied_pipeline.router.highlighted_target
            self.content_label.setText(f"目标: {target} | 状态: {context.status.value}")
    #连接服务器
    ...
    def on_server_data(self, data):
        ...
        if msg == 'ipc_algorithm_test':
            ...
            self.handle_embodied_command(result)
